...
- **Harness** 决定 Agent 在什么阶段、什么范围和什么检查关卡下执行。
...
公开 Skill 只能作为参考，项目内 Skill 必须结合自身业务、架构、工具和检查关卡。
...
先用人工流程和模板验证方法，再封装 Skill；先用少量 Agent 验证交接，再自动编排；先验证检查关卡有效，再构建平台化界面。

## 7. 当前 C1 候选

里程碑 C 的首个候选是 [受控任务验证 Skill](受控任务验证Skill设计.md)：
