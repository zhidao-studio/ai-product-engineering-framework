...
> 概念图由 ChatGPT 生成，用于快速建立认知。正式定义以核心 Markdown、设计决策、契约和 Mermaid 图为准。
...
以下四份文档是后续 Context、Harness、Skills、Agents、Loop、模板、门禁和参考工程的上位依据：
...
高保真预览不是宣传插图，而是编码前的体验门禁；模拟用户验收和发布交付是两个独立阶段；发布不是终点，真实反馈必须进入下一轮产品决策。
...
- **v0.1.3**：完成 YouYu 首轮业务验证事实同步、版本状态修正和 Framework 回写治理；
