...
执行状态：active
业务准备：in_progress
工程规格：候选资产、既有引用扫描与关键参数确认任务已完成；12 项参数待维护者确认，正式迁移与 OpenAPI 尚未建立
...
- 账号、资料、验证码和协议确认候选模型、接口、错误码、字段映射及既有引用扫描已完成；12 项关键参数已进入维护者确认清单。正式迁移、OpenAPI、实现任务边界和三层验证尚未完成，正式业务实现仍为 `blocked`；
