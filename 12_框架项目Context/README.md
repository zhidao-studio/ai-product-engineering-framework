...
| Harness 工程 | [执行控制与检查关卡](../05_Harness工程/执行控制与检查关卡.md) | 已从 YouYu 提取候选检查关卡，待里程碑 B 可执行化 |
...
| 状态、版本和术语一致性依赖人工维护 | 已出现版本与表达漂移 | v0.1.4 已修复，后续自动检查 | 里程碑 B |
...
5. 交叉启动 Harness 里程碑 B，优先解决 PR 状态、人工检查关卡和实际合并权限不一致；
6. 选择一个包含前端、后端、OpenAPI 和 Schema 的小型完整业务切片；
7. 使用端到端任务验证约定、联调、三层验证和 Framework 回写。
