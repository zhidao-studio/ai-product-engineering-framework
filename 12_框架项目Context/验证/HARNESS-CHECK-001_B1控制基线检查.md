...
verifier: Codex
human_approval: approved_for_b1_exit
...
| 人工退出批准 | 维护者在完整 B 阶段方案后授权 Codex 执行 | 通过 |
...
`passed`（仅限 B1 工程范围）：B1 内容、静态检查和执行授权完整，可以进入 B2。该结果不代表 B1 候选规则已经通过 YouYu 真实任务验证，资产成熟度保持 `candidate`。
