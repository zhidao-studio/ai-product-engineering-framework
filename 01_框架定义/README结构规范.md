...
> 本文定义根目录 `README.md` 的信息架构、版本表达和维护边界。README 是人类快速理解项目的首页，不替代宪法、专题文档、正式约定和设计决策。
...
- 将开发中或候选能力写成稳定能力；
- 只更新首页而不更新事实源；
- 使用与 VERSION、CHANGELOG、Roadmap 不一致的版本状态；
...
`v0.2-A` 是建设里程碑，不是发布版本。详细规则链接到 `10_版本演进/版本管理规范.md`。
...
2. 核心模型、版本、边界、术语和责任变化时必须同步；
3. 不堆放平台命令、模板全文和实现细节；
4. 图片、Mermaid 和文字必须表达一致；
5. 版本状态必须与 VERSION、CHANGELOG、Roadmap 和项目 Context 一致；
6. 新链接提交前必须验证；
7. 已删除或替代文档不得继续作为权威导航入口；
8. 真实 Review 发现的问题应进入复核报告，不只改首页；
9. 中文主要表述使用“约定”和“检查关卡”，必要技术名和机器标识按规范保留。
