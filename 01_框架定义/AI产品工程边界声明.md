...
正式权利边界以根目录 `LICENSE` 和 [DEC-009](../11_设计决策/DEC-009_采用专有闭源许可并限制未经授权使用.md) 为准。公开可见不等于授予使用许可；GitHub 已核验当前仓库为 Public，若要限制访问，维护者必须手动改为 Private。
...
- 当前运行状态入口、历史快照边界和 Context 仓库基线治理。

当前进入 v0.2.0 / A 的 Context 可执行化里程碑：

- A1“规范、候选模板与 Framework 自应用”已经完成；
- A2“正式业务参考工程验证准备”当前因 YouYu 基础框架整理处于 `blocked`；
- 正式业务参考工程验证为 `not_started`；
