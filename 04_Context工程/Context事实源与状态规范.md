...
| 字段 | 含义 | 示例 |
|---|---|---|
| `context_pack_version` | 当前 Pack 自身版本 | `0.2-A.2` |
| `project_context_pack_version` | 任务或阶段引用的项目 Pack 版本 | `0.2-A.2` |
| `stage_context_pack_version` | 任务引用的阶段 Pack 版本 | `1.1` |
| `source_version` | 某个产品、设计或契约事实的版本 | `OpenAPI v2` |
| `stable_release` | 已确认稳定的框架或产品发布 | `v0.1.1` |
| `target_release` | 当前工作计划进入的发布 | `v0.2.0` |
| `source_commit` | Context 固定的 Git 基线 | `abc1234` |
...
文档表格可以显示中文，但 YAML、脚本和自动门禁必须使用机器值。不得在同一字段中混用 `active`、`当前有效`、`进行中` 等不同语义。
...
设计决策通常解释“为什么”，专题文档和契约描述“当前是什么”。如决策与当前事实描述不一致，不得只按序号机械覆盖，应检查决策状态、替代关系和适用范围。
...
| CTX-API-001 | 主接口契约 | 工程 | 项目 | openapi.yaml | active | 后端责任人 | 2026-07-12 |
...
- 是否标明敏感级别；
- 是否说明替代了什么；
- 是否需要同步 README、AGENTS、契约、版本记录或设计决策。
