...
| 7. 质量与安全验证 | 系统是否正确、可靠和安全？ | 静态审查、约定一致性检查、运行测试和安全结果 |
...
- 哪些经验可以进入长期 Context、检查关卡或 Skill。
...
4. **检查关卡**：用自动或人工证据证明产物符合标准；
5. **平台适配**：将平台无关标准映射到 Claude Code、Codex、Kimi、GLM 等环境；
6. **参考工程**：用真实项目验证框架并反向产生改进。

### 7.1 横向专业能力

三平面和十阶段之外，Framework 可以建立跨阶段的专业工程能力。它们不新增生命周期阶段，也不改变五大 AI 工程基础设施。

首个专业能力是 [体验与设计工程](../13_体验与设计工程/README.md)，贯穿用户体验设计、高保真、工程规格、执行、验证和模拟用户验收，负责：

- 统一体验意图、信息架构和状态语义；
- 将品牌与 Design Tokens 映射到不同客户端；
- 为 iOS、Android、桌面、Web 和 H5 选择符合平台习惯的组件与交互；
- 建立页面、高保真、真实实现和用户验收之间的可追溯关系。

## 8. 单一事实源规则

- 全局模型只在本文件定义；
- 愿景、场景、原则、边界和术语规则分别由 `01_框架定义` 下对应文档定义；
- 设计原因由 `11_设计决策` 保存；
